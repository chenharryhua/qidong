...
it is still underconstruction. clone it from the link

# First Program
Suppose that we have tree functions:
```scala
val f1 = (i: Int) => i + 1
val f2 = (i: Int) => i + 2
val f3 = (i: Int) => i + 3
...
```
the return value `task` is still a value which has the type of `Task[\/[MFailure[Task, MSuccess[Int]], MSuccess[Int]]]`. I will talk about the type in Resume section. To evaluate it, we call one of Scalaz Task's `unsafe-*` methods:
```scala
val \/-(ret) = task.unsafePerformSync
```
if all good, we eventually get back a value of type `\/-[MSuccess[Int]]`.
`MSucess` is defined as:
```scala
final case class MSuccess[IO](trace: Tree[MTraceNode], data: IO)
```
the `data` is the return value of evaluating the whole expression. the `trace` looks like this:
```
MRoot
...
Scala function would throw exceptions:
```scala
val f1 = ((i: Int) => i + 1).name("function 1")
val f2 = ((i: Int) => {throw new Exception("haha"); i + 2}).name("function 2")
val f3 = ((i: Int) => i + 3).name("function 3")
val fs = f1 =>: f2 =>: f3
...
+- MFailNode(function 2,Timing(2017-06-02T13:37:04.837,2017-06-02T13:37:04.837))
`- MNotRunNode(function 3)
```
beside `MSuccNode`, there are other two which are worthing to talk about. The `MFailNode` indicates which functions is failed, when it is failed and why it is failed(the exception). The `MNotRunNode` tells which functions are not running because of previous failure.
so, the `trace` gives us the whole runtime log when run/evaluate the expression.

## Type safety
The `=>:` combinator can compose any function in a sensible way. Compilation error will occurs when two function is not compatiable. in terms of compatiable I mean that the first function's return type should be subtype of the second function's input type.
```scala
   ...
   import qidong.pipeline.ops._
    ...
    illTyped("""m2 =>: m3""")
```
`m1` can compose with `m2` because `m1`'s return type(String) is compatiable to `m2`'s input type(String).
`m2` can not compose with `m3` because `m2`'s return type(Int) is incompatiable to `m3`'s input type(String). Incompatiable combination will fail the compiler.
...
```scala
    ...
    val fs = f1 =>: f2
```
`f1` and `f2` are composable because f1's return type( C ) is subtype of f2's input type(B).

two classes of functions `() => A` and `Unit => A`, where `A` is a type parameter, are internally transformed to `Any => A`. `Any` is the top type in Scala so that they can follow any other functions.
```scala
    val m1 = () => 1
    val m2 = (Unit: Unit) => 1
...
## Effectful type
functions have the shape `I => F[O]` is effectful when F would be one of the following type:
- Either[Throwable, ?]
- Try
- \\/[Throwable,?]
- Future
...
## Adaptation
when we want to compose two functions where the subtype relation is not hold, Qidong provides a few methods to adjust the input of function or the output of function as per your need.
suppose we have two functions in hand:
```scala
    import shapeless.test.illTyped
...
### keep
the `keep` method is needed when the second function also want to access the first function's input.
```scala
    val m1 = (i: Int) => i
...
```
### long distance keep
when we want to access some data not immediate before:
```scala
    val m1 = (i: Int) => i
    val m2 = (i: Int, j: Int) => i + j
...
## Resume computation
the return type of evaluation of an expression is `\/[MFailure[E, MSuccess[A]], MSuccess[A]]`. the `Mfailure` data type looks like:
```scala
...
```
it may be useful for a long run task which would fail potentially but you don't want to re-run whole task from the very begining. 
...
```
when you give a name to a few of functions, you create a logic group.
