...
object ops {
  ...
  implicit final class MsOps[M2, M2Out <: HList](m2: M2)
      extends Serializable {
    final def =>:[M1, M1Out](m1: M1)(
      implicit lc1: ListOfCaspers.Aux[M1, M1Out],
    ...
    final def mapsndFlatTuple[F[_], I, TO, O, O2, ML <: HList, SOut <: HList](f: O => O2)(
      implicit lc: ListOfCaspers.Aux[M2, M2Out],
    ...
    final def mapFlatTuple[F[_], I, TO, O, O2, ML <: HList, SOut <: HList](f: O => O2)(
      implicit lc: ListOfCaspers.Aux[M2, M2Out],
      lastOf: LastOf[M2, F, I, TO],
      mf: MapSnd.Aux[M2Out, TO, O2, SOut],
      ft: FlattenTuple.Aux[TO, ML],
      tupler: Tupler.Aux[ML, O]): SOut = mapsndFlatTuple(f)
    ...
    final def tree(implicit callgraph: CallGraph[M2]): Tree[String] =
      callgraph(m2, Node(MRoot.name, Stream()))
    final def drawTree(implicit callgraph: CallGraph[M2]): String =
      callgraph(m2, Node(MRoot.name, Stream())).drawTree

    protected final class EvalMMsHelper[E[_]] {
      def apply[I](i: I)(
        implicit env: EvalCap[E],
        decomposer: eval.Decomposer[M2, E, I]): decomposer.Out = {
