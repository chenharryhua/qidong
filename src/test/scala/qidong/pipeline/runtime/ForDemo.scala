...
class ForDemo extends FunSuite {
  def f0(i: Int): Future[Int] = Future(i + 1)
  def f1(i: Int): Try[Int] = Try(i + 1)
  def f2(i: Int): \/[Throwable, Int] = (i + 1).right[Throwable]
  def f3(i: Int): Either[Throwable, Int] = Right[Throwable, Int](i + 1)
  def f4(i: Int): Int = i + 1
  def f5(i: Int): Option[Int] = i.some
  def fex(i: Int): Int = { throw new Exception("haha"); i + 1 }
  var flag = 0
  def frecover(i: Int): Int = if (flag == 0) {
    flag += 1
  ...
  test("arbitarily compose") {
    val ms = m0 =>: m1 =>: m2.stateUpdate(println) =>: m3 =>: m4 =>: m5 //=>: incompatible
    //show structure
    println(ms.drawTree)
